...
_Last updated: **2026-09-23 ~21:40 UK** · branch `alpha` · tree clean after this commit_
...
- **This session (23 Sep):** refreshed the standing rules and this handoff so a brand-new
  session can pick up cleanly. **No app code was changed.**
...
- **Q3. Codex and the dev-team plugins aren't installed in this cloud computer** (they are
  switched on in `.claude/settings.json`, but not present here). So Codex reviews need to
  run from your Mac (or the cloud environment needs them added). OK to keep it that way?
