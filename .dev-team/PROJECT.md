...
2. ✅ **STABILIZE-2 DONE** (cycle 4): +48 safety-net tests (342→390 green). Surfaced 2 HIGH — **B-027** (`Database::insert()` missing → WebAuthn registration broken) + **FG-013** (WebAuthn accepts unsigned assertions — auth-bypass, PoC-confirmed). **SAFETY FLOOR → next cycles (5–6) fix WebAuthn BEFORE the feature campaign:** ✅ **cycle 5 DONE**: B-027 (added `Database::insert()`/`insertId()` — unbroke 7 callers incl. PasswordlessLogin + 4 email + AccountManager) + B-028 (fixtures→canonical lowercase `active`; BaseController already correct). ✅ **cycle 6 DONE**: FG-013 closed (real WebAuthn verification — CBOR/COSE→PEM, openssl_verify, sign-count clone-detect; **red-team HOLDS** across 7 vectors; +2 pre-existing bug fixes). **NEXT cycle 7 = B-029** (stand up test DB, triage ~63 integration errors → fix real bugs; HIGH, blocks "core task works e2e"). THEN WebAuthn residuals B-030/B-031/B-032 + SECURE (B-024 CORS, 11 deferred #22–#34/#73). THEN feature campaign G-003 → G-001 → G-002 → G-004.
...
- B-032 | Usernameless/discoverable-credential WebAuthn login fails closed: `generateAuthenticationOptions(null)` stores userID=NULL challenge but verify requires the credential owner's userID → valid usernameless assertion rejected (availability, fail-safe) | correctness | Med | M | (WebAuthnHandler.php; yes) | — (red-team cycle 6) | while app/fixtures use `'Active'` — API auth path would fail to resolve users on a case-sensitive (`_bin`) collation | robustness | Low | S | (BaseController.php; yes) | — (found cycle 4) |
