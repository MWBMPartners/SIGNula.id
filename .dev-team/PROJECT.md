...
- ✅ **B-028 DONE (c5)** — premise INVERTED: canonical is lowercase `'active'` (the ENUM + all app code); BaseController was already correct. Real drift was in test fixtures → aligned to lowercase. ORIG (as mis-stated): `BaseController::getUserById()/getUserByApiKey()` filter `accountStatus='active'` (lowercase) while app/fixtures use `'Active'` — API auth path would fail to resolve users on a case-sensitive (`_bin`) collation | robustness | Low | S | (BaseController.php; yes) | — (found cycle 4) |
...
| 5 | STABILIZE | B-027 + B-028: add `Database::insert()`/`insertId()` (missing, fataling 7 callers); align fixtures to canonical lowercase accountStatus | 390 green; UNBROKE PasswordlessLogin + 4 email features + AccountManager + WebAuthn storage with one helper; BaseController already correct (premise inverted) | cycle-5 commit |
