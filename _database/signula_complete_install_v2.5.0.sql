...
-- ============================================================================
-- 📧 TABLE: tblEmailTemplates  (restored — required as FK parent)
-- ============================================================================
CREATE TABLE IF NOT EXISTS `tblEmailTemplates` (
    ...
    -- [install-fix] columns added by migration 001 ALTERs (which use MariaDB-only
    -- ADD COLUMN IF NOT EXISTS and never applied on MySQL). Declared up front so
    -- the installer's tblEmailTemplates seed INSERTs succeed.
    `category` VARCHAR(50) NULL COMMENT 'Template category (auth, payment, marketing, ...)',
    `trackingEnabled` BOOLEAN NOT NULL DEFAULT FALSE COMMENT 'Whether open/click tracking is enabled',
    `requiredVariables` JSON NULL COMMENT 'JSON array of required template variables',
    `variableDescriptions` JSON NULL COMMENT 'JSON map of variable -> description',
    `version` INT UNSIGNED NOT NULL DEFAULT 1 COMMENT 'Template version number',
    `previousVersionID` BIGINT UNSIGNED NULL COMMENT 'Prior version (self-reference)',
    `metadata` JSON NULL COMMENT 'Additional template metadata',
    `defaultAttachments` JSON NULL COMMENT 'Array of default attachments',
    ...
    `updatedBy` BIGINT UNSIGNED NULL COMMENT 'User who last updated template',
...
-- ============================================================================
-- 📧 TABLE: tblEmailQueue  (restored — required as FK parent)
-- ============================================================================
CREATE TABLE IF NOT EXISTS `tblEmailQueue` (
    `emailID` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY COMMENT 'Unique queue entry identifier',
    ...
    `replyToEmail` VARCHAR(255) NULL COMMENT 'Reply-to email',
    ...
    `scheduledAt` DATETIME NULL COMMENT 'Scheduled send time (NULL = send ASAP)',
    ...
    INDEX `idx_createdAt` (`createdAt`),
    CONSTRAINT `fk_emailqueue_user` FOREIGN KEY (`userID`) REFERENCES `tblUsers`(`userID`) ON DELETE SET NULL ON UPDATE CASCADE,
    CONSTRAINT `fk_emailqueue_template` FOREIGN KEY (`templateID`) REFERENCES `tblEmailTemplates`(`templateID`) ON DELETE SET NULL ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Email queue';

-- ============================================================================
-- 🔐 TABLE: tblUserSessions  (restored — app + tests target this name, not tblSessions)
-- ============================================================================
CREATE TABLE IF NOT EXISTS `tblUserSessions` (
    ...
    INDEX `idx_isActive` (`isActive`),
    CONSTRAINT `fk_usersessions_user` FOREIGN KEY (`userID`) REFERENCES `tblUsers`(`userID`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Active user sessions';

-- ============================================================================
-- 🎟️ TABLE: tblVerificationTokens  (restored — app + tests target this table)
-- ============================================================================
CREATE TABLE IF NOT EXISTS `tblVerificationTokens` (
    ...
    CONSTRAINT `fk_verificationtokens_user` FOREIGN KEY (`userID`) REFERENCES `tblUsers`(`userID`) ON DELETE CASCADE ON UPDATE CASCADE
...
-- ----------------------------------------------------------------------------
-- [install-fix] replaced STALE duplicate tblWebAuthnCredentials definition with the app-correct shape (WebAuthnHandler/migration 005)
CREATE TABLE IF NOT EXISTS `tblWebAuthnCredentials` (
    ...
    CONSTRAINT `fk_webauthn_credential_user` FOREIGN KEY (`userID`) REFERENCES `tblUsers`(`userID`) ON DELETE CASCADE ON UPDATE CASCADE
...
-- ----------------------------------------------------------------------------
-- [install-fix] replaced STALE duplicate tblWebAuthnChallenges definition with the app-correct shape (WebAuthnHandler/migration 005)
CREATE TABLE IF NOT EXISTS `tblWebAuthnChallenges` (
    ...
    CONSTRAINT `fk_webauthn_challenge_user` FOREIGN KEY (`userID`) REFERENCES `tblUsers`(`userID`) ON DELETE CASCADE ON UPDATE CASCADE
...
-- ----------------------------------------------------------------------------
-- [install-fix] tblActivityLog replaced with the app-correct (ActivityLogger/ErrorLogger-matching) shape
...
-- ----------------------------------------------------------------------------
-- [install-fix] tblErrorLog replaced with the app-correct (ActivityLogger/ErrorLogger-matching) shape
...
-- NOTE: Seed data for tblEmailABTests and tblEmailABTestVariants
-- should be added here if needed for initial setup.
...
-- [install-fix] guarded OAuth multi-account ALTERs (idempotent)
SET @__c := (SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='tblOAuthAccounts' AND COLUMN_NAME='accountType');
SET @__s := IF(@__c=0, "ALTER TABLE tblOAuthAccounts ADD COLUMN accountType VARCHAR(50) NULL COMMENT 'Account type (personal/workspace/etc)'", 'DO 0'); PREPARE __s FROM @__s; EXECUTE __s; DEALLOCATE PREPARE __s;
SET @__c := (SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='tblOAuthAccounts' AND COLUMN_NAME='emailDomain');
SET @__s := IF(@__c=0, "ALTER TABLE tblOAuthAccounts ADD COLUMN emailDomain VARCHAR(255) NULL", 'DO 0'); PREPARE __s FROM @__s; EXECUTE __s; DEALLOCATE PREPARE __s;
SET @__c := (SELECT COUNT(*) FROM information_schema.STATISTICS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='tblOAuthAccounts' AND INDEX_NAME='idx_emailDomain');
SET @__s := IF(@__c=0, "ALTER TABLE tblOAuthAccounts ADD INDEX idx_emailDomain (emailDomain)", 'DO 0'); PREPARE __s FROM @__s; EXECUTE __s; DEALLOCATE PREPARE __s;
SET @__c := (SELECT COUNT(*) FROM information_schema.STATISTICS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='tblOAuthAccounts' AND INDEX_NAME='uq_provider_external_account');
SET @__s := IF(@__c=0, "ALTER TABLE tblOAuthAccounts ADD CONSTRAINT uq_provider_external_account UNIQUE (provider, providerUserID)", 'DO 0'); PREPARE __s FROM @__s; EXECUTE __s; DEALLOCATE PREPARE __s;
...
CREATE TABLE IF NOT EXISTS tblContactSubmissions (
    -- Primary Key
    submissionID BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY COMMENT 'Unique identifier for each contact submission',

    -- Contact Information
    name VARCHAR(100) NOT NULL COMMENT 'Full name of person submitting',

    email VARCHAR(255) NOT NULL COMMENT 'Email address for response',

    -- Inquiry Details
    inquiryType ENUM('general', 'support', 'sales', 'partnership', 'feedback') NOT NULL DEFAULT 'general' COMMENT 'Type of inquiry to route to appropriate team',

    subject VARCHAR(200) NOT NULL COMMENT 'Subject line of the inquiry',

    message TEXT NOT NULL COMMENT 'Full message content (up to 2000 characters in form)',

    -- Status Tracking
    status ENUM('new', 'in_progress', 'responded', 'resolved', 'spam') NOT NULL DEFAULT 'new' COMMENT 'Current status of the submission',

    priority ENUM('low', 'normal', 'high', 'urgent') NOT NULL DEFAULT 'normal' COMMENT 'Priority level assigned by staff',

    -- Assignment & Follow-up
    assignedTo BIGINT UNSIGNED NULL DEFAULT NULL COMMENT 'userID of staff member assigned to this inquiry',

    responseNotes TEXT NULL COMMENT 'Internal notes about the response',

    respondedAt DATETIME NULL DEFAULT NULL COMMENT 'When response was sent',

    respondedBy BIGINT UNSIGNED NULL DEFAULT NULL COMMENT 'userID of staff member who responded',

    -- Security & Tracking
    ipAddress VARCHAR(45) NOT NULL COMMENT 'IPv4 or IPv6 address of submitter',

    userAgent VARCHAR(500) NULL COMMENT 'Browser user agent string',

    referrer VARCHAR(500) NULL COMMENT 'Page that referred to contact form',

    -- Spam Detection
    spamScore DECIMAL(3, 2) NULL DEFAULT NULL COMMENT 'Spam probability score (0.00 to 1.00)',

    isSpam BOOLEAN NOT NULL DEFAULT FALSE COMMENT 'Flagged as spam',

    -- Timestamps
    submittedAt DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'When form was submitted',
...
-- ============================================
-- [install-fix] removed demo tblBlogPosts seed (requires an existing author user; not needed for a clean install)
...
-- [install-fix] removed now-duplicate tblWebAuthnCredentials definition (app-correct shape already defined above)
...
-- [install-fix] removed now-duplicate tblWebAuthnChallenges definition (app-correct shape already defined above)
...
-- [install-fix] guarded (sendAsEmail already added to tblEmailQueue above)
SET @__c := (SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='tblEmailQueue' AND COLUMN_NAME='sendAsEmail');
SET @__s := IF(@__c=0, "ALTER TABLE `tblEmailQueue` ADD COLUMN `sendAsEmail` VARCHAR(255) NULL COMMENT 'Delegate mailbox to send from (if different from fromEmail)' AFTER `fromName`", 'DO 0'); PREPARE __s FROM @__s; EXECUTE __s; DEALLOCATE PREPARE __s;
SET @__c := (SELECT COUNT(*) FROM information_schema.STATISTICS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='tblEmailQueue' AND INDEX_NAME='idx_send_as');
SET @__s := IF(@__c=0, "ALTER TABLE `tblEmailQueue` ADD INDEX `idx_send_as` (`sendAsEmail`)", 'DO 0'); PREPARE __s FROM @__s; EXECUTE __s; DEALLOCATE PREPARE __s;
...
-- [install-fix] removed incompatible self-bookkeeping INSERT into tblMigrations (runner tracks migrations)
...
-- [install-fix] ensure tblPartners.status exists
SET @__hasStatus := (SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='tblPartners' AND COLUMN_NAME='status');
SET @__sql := IF(@__hasStatus=0, "ALTER TABLE tblPartners ADD COLUMN `status` ENUM('pending','active','suspended','rejected') NOT NULL DEFAULT 'active' COMMENT 'Partner account status'", 'DO 0');
PREPARE __st FROM @__sql; EXECUTE __st; DEALLOCATE PREPARE __st;
...
-- ============================================================================
-- 🏢 MULTI-TIER ADMIN SYSTEM (Migration 009)
-- ============================================================================
...
-- Partner team members with roles
...
-- Partner-specific feature overrides
...
-- Assign root admin for existing partners
-- [install-fix] removed 'assign root admin for existing partners' seed (no partners exist at fresh install; referenced non-existent columns)

-- Partner directory and team size settings
...
-- Root admin enforcement triggers
DELIMITER //
...
-- [install-fix] removed VIEW referencing non-existent tblPartners columns (companyName/email); app queries in PHP
...
-- ============================================================================
-- 💰 WEBHOOKS & PAYMENT SYSTEM (Migration 010)
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS `tblSubscriptionTiers` (
    ...
    `isCustom` TINYINT(1) NOT NULL DEFAULT 0 COMMENT 'Whether this is a custom client-created tier (1) or default global tier (0)',
    `createdByPartnerID` BIGINT UNSIGNED DEFAULT NULL COMMENT 'Partner who created this custom tier (NULL for global tiers)',
    `parentTierID` BIGINT UNSIGNED DEFAULT NULL COMMENT 'Base tier this custom tier is derived from (NULL for standalone/global)',
    `usageLimits` JSON DEFAULT NULL COMMENT 'Per-metric usage limits for this tier (e.g. {"api_calls": 5000, "storage_gb": 10})',
    `billingMode` ENUM('fixed', 'usage', 'hybrid') NOT NULL DEFAULT 'fixed' COMMENT 'Default billing mode for subscriptions on this tier',
    ...
    INDEX `idx_st_custom` (`isCustom`, `createdByPartnerID`),
    INDEX `idx_st_billing_mode` (`billingMode`),
    CONSTRAINT `fk_st_created_by_partner` FOREIGN KEY (`createdByPartnerID`) REFERENCES `tblPartners` (`partnerID`) ON DELETE SET NULL ON UPDATE CASCADE,
    CONSTRAINT `fk_st_parent_tier` FOREIGN KEY (`parentTierID`) REFERENCES `tblSubscriptionTiers` (`tierID`) ON DELETE SET NULL ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
COMMENT='Subscription tier definitions with pricing and feature limits';

-- Default subscription tiers (Free → Basic → Premium → Pro → Platinum → Enterprise)
...
VALUES
    ...
    ('Pro', 'pro', 'Professional-grade features for growing businesses needing higher capacity and advanced tools', 49.99, 499.99, 'GBP', '["Everything in Premium","50 team members","200,000 API calls/month","Phone & email support","Custom webhooks","Advanced security policies","Role-based access control","API rate limit customisation","Usage analytics dashboard","Bulk user management"]', '{"api_calls": 200000, "team_members": 50, "storage_gb": 100}', 50, 200000, 4, 1, 0, 14, NULL, NULL),
    ('Platinum', 'platinum', 'Near-unlimited capacity with premium support and advanced compliance features', 79.99, 799.99, 'GBP', '["Everything in Pro","200 team members","500,000 API calls/month","Dedicated account manager","Priority phone support","SAML SSO","SCIM provisioning","Advanced audit logging","Compliance reports","Custom branding & white-label","Data residency options"]', '{"api_calls": 500000, "team_members": 200, "storage_gb": 500}', 200, 500000, 5, 1, 0, 30, 'Best Value', 'success'),
    ('Enterprise', 'enterprise', 'Unlimited access with dedicated support and SLAs', 99.99, 999.99, 'GBP', '["Everything in Platinum","Unlimited team members","Unlimited API calls","Dedicated support","Custom SLAs","On-premise deployment option","SAML/SCIM integration","Custom contracts"]', '{"api_calls": null, "team_members": null, "storage_gb": null}', NULL, NULL, 6, 1, 0, 30, 'Custom', 'dark');
...
-- ============================================================================
-- 💳 PAYMENT PROVIDER INTEGRATION (Migration 011)
-- ============================================================================
...
-- ============================================================================
-- 🏦 TWO-TIER PAYMENT EXPANSION (Migration 012)
-- ============================================================================
...
-- ============================================================================
-- 🎨 KO-FI & PATREON INTEGRATION (Migration 013)
-- ============================================================================
...
-- ============================================================================
-- ✅ INSTALLATION COMPLETE (v2.5.0-beta)
-- ============================================================================
